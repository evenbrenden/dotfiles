{ lib, stdenv, directoryListingUpdater, fetchurl, alsa-lib, alsa-plugins, gettext, makeWrapper, ncurses, libsamplerate
, pciutils, procps, which, fftw, pipewire, withPipewireLib ? true, symlinkJoin, }:
...
in stdenv.mkDerivation rec {
  ...
  postFixup = ''
    ...
    wrapProgram $out/bin/alsabat --set-default ALSA_PLUGIN_DIR ${plugin-dir}
    wrapProgram $out/bin/alsamixer --set-default ALSA_PLUGIN_DIR ${plugin-dir}
    wrapProgram $out/bin/aplay --set-default ALSA_PLUGIN_DIR ${plugin-dir}
  '';

  passthru.updateScript = directoryListingUpdater { url = "https://www.alsa-project.org/files/pub/utils/"; };
